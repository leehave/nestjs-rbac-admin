...
import { MindDocumentPageDto } from './dto';
...
@Injectable()
export class MindDocumentService {
  constructor(
    ...
    private readonly indexQueue: MindDocumentIndexQueueService,
   ...
   * @returns 索引任务
   */
  private toJob(row: MindSystemDocumentEntity): DocumentIndexJob {
    return {
      documentId: row.id,
   ...
   */
  private async mapRecord(row: MindSystemDocumentEntity) {
   ...
   */
  async page(dto: MindDocumentPageDto) {
