...
@Injectable()
export class MindUserService {
  constructor(
    @InjectRepository(UserEntity)
    private readonly userRepo: Repository<UserEntity>,
    @InjectRepository(SysUserTenantEntity)
    private readonly userTenantRepo: Repository<SysUserTenantEntity>,
    @InjectRepository(MindUserProfileEntity)
    private readonly profileRepo: Repository<MindUserProfileEntity>,
    @InjectDataSource()
   ...
   */
  private toRow(entity: UserEntity, profile?: MindUserProfileEntity | null) {
    return {
      id: String(entity.id),
   ...
   */
  async page(dto: MindUserPageDto) {
    ...
    const profiles = userIds.length
      ...
      : [];
    const profileMap = new Map<number, MindUserProfileEntity>();
   ...
   * @returns 无返回值
   */
  async create(dto: MindUserCreateDto) {
    const tenantId = this.requireTenantId();
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      const profile = manager.create(MindUserProfileEntity, {
        tenantId,
        userId: saved.id,
        resume: dto.resume ?? null,
        photo: dto.photo ?? null,
      });
      await manager.save(MindUserProfileEntity, profile);
    });
  }
   ...
   * @returns 无返回值
   */
  async update(dto: MindUserUpdateDto) {
    const tenantId = this.requireTenantId();
   ...
   * @returns 无返回值
   */
  async remove(dto: MindUserDeleteDto) {
    const tenantId = this.requireTenantId();
   ...
   */
  async select(dto: MindUserSelectDto) {
    const tenantId = this.requireTenantId();
