...
@Injectable()
export class MindGraphService {
  private readonly logger = new Logger(MindGraphService.name);
  ...
  private getNeo4jConfig() {
    const url = this.configService.get<string>('mind.neo4j.boltUrl') || '';
    const username = this.configService.get<string>('mind.neo4j.username') || '';
    const password = this.configService.get<string>('mind.neo4j.password') || '';
  ...
  getLabelPrefix(tenantId: number, libraryNumber: string) {
    const prefix =
      this.configService.get<string>('mind.neo4j.labelPrefix') ||
      this.configService.get<string>('mind.qdrant.collectionPrefix') ||
      'mind_rag_';
  ...
  /**
   * 对齐 mind graph_store：用 LLM 抽取实体关系图并写入 Neo4j。
   * 节点 type 前缀化（tenant_library）以隔离不同知识库。
   * ponytail: 需要 Neo4j 已安装 APOC（与 mind 一致）；跨库相同实体 id 会合并到同一节点，属可接受上限。
   */
  async addDocumentGraph(
    ...
    const { url, username, password } = this.getNeo4jConfig();
    if (!url || !username) {
      this.logger.warn('Neo4j graph extraction skipped: MIND_NEO4J_BOLT_URL or MIND_NEO4J_USERNAME is empty');
      return;
    }
    ...
    // 并行抽取（库自带 convertToGraphDocuments 是串行，这里用并发池对齐 mind ThreadPoolExecutor）
