...
import { MindLlmService } from '../llm/mind-llm.service';
import { MindLlmRuntimeService } from '../llm/mind-llm-runtime.service';
import { normalizeMindDistance } from '../llm/mind-llm-config.util';

type QdrantDistance = 'Cosine' | 'Euclid' | 'Dot';

@Injectable()
export class MindVectorService {
  constructor(
   ...
   */
  private get client() {
    const url = this.configService.get<string>('mind.qdrant.url') || '';
    if (!url) return null;
    const timeout = Number(this.configService.get<number>('mind.qdrant.timeout') ?? 30);
  ...
  getCollectionName(tenantId: number, libraryNumber: string) {
    const prefix = this.configService.get<string>('mind.qdrant.collectionPrefix') || 'mind_rag_';
  ...
  async upsertDocumentChunks(
    ...
    const envDim = Number(this.configService.get<number>('mind.qdrant.vectorSize') ?? 1024);
    const vectorSize = Number.isFinite(settingDim) && settingDim > 0 ? settingDim : envDim;
    const distance = normalizeMindDistance((opts?.rag as any)?.distance);
    ...
    const batchSize = Math.max(1, Number(this.configService.get<number>('mind.qdrant.embedBatch') ?? 64));
